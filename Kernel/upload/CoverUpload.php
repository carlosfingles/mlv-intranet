...
if (isset($_FILES["file"]))
{
    ...
    $size = $file["size"];
    $limite = 1024 * 1024;
    $carpeta = "../../images/imgcover/";
    
    $varExt = explode(".", $file['name']);
    $extension = end($varExt);
    $nombre='Cover_' .date('Y-m-d'). '_' .date('H.i.s');
    ...
    {
        if( file_exists( "../../images/imgcover/".$nombre. '.' .$extension) ){
            ...
            if($size <= $limite){
                ...
                if($thumb->error) {
                    ...
                    {
                        $av = fetch_assoc($avU);
                        if($av['cover']!='cover.jpg'){
                            unlink("../../images/imgcover/".$av['cover']);
                        }
                    }
